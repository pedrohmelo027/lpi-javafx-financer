...
public class RelatorioMensalFactory implements RelatorioFactory {
    private final int mes;
    private final int ano;

    public RelatorioMensalFactory(int mes, int ano) {
        this.mes = mes;
        this.ano = ano;
    }
    ...
    public Relatorio criarRelatorio(List<Transacao> todasTransacoes) {
        List<Transacao> filtradas = todasTransacoes.stream()
                .filter(t -> t.getDate() != null && t.getDate().getMonthValue() == mes && t.getDate().getYear() == ano)
        ...
        String titulo = String.format("Relatório Mensal - %02d/%d", mes, ano);
